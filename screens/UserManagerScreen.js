...
const UserManagerScreen = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://13.213.1.45:8080/api/auth/users');
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.get(`http://13.213.1.45:8080/api/auth/users/search?name=${searchQuery}`);
      setUsers(response.data.data);
    } catch (error) {
    ...
    Alert.alert(
      ...
      [
        { text: 'Hủy', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const response = await axios.delete(`http://13.213.1.45:8080/api/auth/users/${userId}`);
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://13.213.1.45:8080/api/auth/users/${selectedUser.id}`, editForm);
